...
internal fun SettingsScreen(
    ...
    Column(
        ...
        Column(
            ...
            SettingsSectionTitle("视频来源")
            VideoInputSelector(
                selected = settings.videoInputSource,
                onSelected = { source ->
                    onSettingsChanged(settings.copy(videoInputSource = source))
                },
            )

            if (settings.videoInputSource == VideoInputSource.GLASSES) {
                SettingIconLabel(Icons.Default.Wifi, "眼镜热点配网")
                Text(
                    text = "仅在眼镜尚未保存当前手机热点时使用",
                    color = AppMutedText,
                    style = MaterialTheme.typography.bodySmall,
                    modifier = Modifier.padding(bottom = 6.dp),
                )
                BleProvisioningSettings()
            }
            ...
            InformationRow(
                icon = if (settings.videoInputSource == VideoInputSource.GLASSES) {
                    Icons.Default.Videocam
                } else {
                    Icons.Default.PhoneAndroid
                },
                title = "画面输入",
                value = if (settings.videoInputSource == VideoInputSource.GLASSES) {
                    "眼镜 WebRTC"
                } else {
                    "本机摄像头"
                },
            ...
            InformationRow(title = "应用版本", value = "1.21")
...
@Composable
private fun VideoInputSelector(
    selected: VideoInputSource,
    onSelected: (VideoInputSource) -> Unit,
) {
    val options = listOf(
        VideoInputSource.GLASSES to "眼镜",
        VideoInputSource.PHONE_CAMERA to "手机",
    )

...
class GroundClassificationRendererTest {
    ...
    @Test
    fun fasterPreviewThrottleAllowsTenFramesPerSecond() {
        val throttle = ClassificationRenderThrottle(intervalMs = 100L)

        assertTrue(throttle.shouldRender(enabled = true, nowMs = 1_000L))
        assertFalse(throttle.shouldRender(enabled = true, nowMs = 1_099L))
        assertTrue(throttle.shouldRender(enabled = true, nowMs = 1_100L))
    }

    @Test
    fun resetForcesTheNextFrameToRender() {
        val throttle = ClassificationRenderThrottle(intervalMs = 250L)

        assertTrue(throttle.shouldRender(enabled = true, nowMs = 1_000L))
        assertFalse(throttle.shouldRender(enabled = true, nowMs = 1_100L))
        throttle.reset()
        assertTrue(throttle.shouldRender(enabled = true, nowMs = 1_101L))
    }
}
